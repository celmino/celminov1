...
export  class LinkTool {
  ...
  render() {
    ...
    this.nodes.container = this.make('div', this.CSS.container as any);
  ...
  makeInputHolder() {
    ...
    this.nodes.input = this.make('div', [this.CSS.input, this.CSS.inputEl] as any, {
  ...
  startFetching(event) {
    ...
    if (event.type === 'paste') {
      url = (event.clipboardData || (window as any).clipboardData).getData('text');
    }
   ...
   * @returns {HTMLElement}
   */
  make(tagName, classNames: any = null, attributes = {}) {
